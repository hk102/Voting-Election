...
contract("Election", async accounts => {
  ...
  it("allows a voter to cast a vote", async() => {
    ...
    assert.equal(receipt.logs[0].event,"votingEvent","the event type is correct");
    assert.equal(receipt.logs[0].args._candidateID.toNumber(),candidateID,"the candidate id is correct");

    let voted = await electionInstance.voters(account[0]);
    assert.equal(voted, "the voter was marked as voted");

    let candidate = await electionInstance.candidates(candidateId);
    let voteCount = candidate[2];

    assert.equal(voteCount,1,"Increment the candidates vote coiunt");
